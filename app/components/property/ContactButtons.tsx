...
const ContactButtons: React.FC<ContactButtonsProps> = ({ property }) => {
  ...
  return (
    <div className="flex items-center justify-center gap-5 md:mb-5">
      {/* WhatsApp Button */}
      <a 
        href={whatsappUrl} 
        target="_blank" 
        rel="noopener noreferrer"
        className="flex gap-2 cursor-pointer text-[12px] md:text-[15px] items-center bg-[#0A2F1E] text-white py-2 px-5 rounded-full"
      >
        <FaWhatsapp size={15} />
        Make enquiries
      </a>
      ...
      <a 
        href={mailtoUrl} 
        className="flex gap-2 text-[12px] cursor-pointer items-center md:text-[15px] bg-[#0A2F1E] text-white py-2 px-5 rounded-full"
      >
        <Copy size={15} />
        Make enquiries
      </a>
